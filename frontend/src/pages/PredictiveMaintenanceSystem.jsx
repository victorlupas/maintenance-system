...
import { AlertTriangle, CheckCircle, Activity, TrendingUp, Calendar, Settings, Download, Upload } from 'lucide-react';
...
const PredictiveMaintenanceSystem = () => {
  ...
  const generateSyntheticData = () => {
    ...
    equipmentProfiles.forEach(eq => {
      for (let i = 0; i < 100; i++) {
        ...
        // Add anomalies for critical/warning equipment
        const anomalyFactor = (eq.health === 'critical' && i > 80) ? 1.15 :
                            (eq.health === 'warning' && i > 90) ? 1.08 : 1;
  ...
  // Anomaly detection using statistical methods and rules
  const performAnomalyDetection = (data, equipmentProfiles) => {
    const detectedAnomalies = [];

    equipmentProfiles.forEach(eq => {
      const eqData = data.filter(d => d.equipmentId === eq.id).slice(-24); // last 24 hours
      ...
      eqData.forEach(reading => {
        ...
        if (anomalyScore > 0.3) {
          detectedAnomalies.push({
            ...
            severity: anomalyScore > 0.6 ? 'critical' : anomalyScore > 0.3 ? 'warning' : 'info',
            issues: issues,
    ...
    setAnomalies(detectedAnomalies.slice(-20)); // keep last 20
  ...
  const generatePredictions = (data, equipmentProfiles) => {
    ...
    equipmentProfiles.forEach(eq => {
      ...
      predictionResults.push({
        ...
        riskLevel: riskLevel,
        recommendedAction: daysToFailure < 15 ? 'Schedule immediate maintenance' :
                          daysToFailure < 30 ? 'Plan maintenance within 2 weeks' :
                          'Continue monitoring',
        trends: {
        ...
        estimatedCost: riskLevel === 'critical' ? 5000 + Math.random() * 3000 :
                       riskLevel === 'medium' ? 2000 + Math.random() * 2000 :
  ...
  // Generate alerts
  const generateAlerts = (anomalies) => {
  ...
  const getChartData = (equipmentId) => {
    return sensorData
      ...
      .map(d => ({
        ...
        vibration: d.vibration.toFixed(2),
      ...
      <div className="max-w-7xl mx-auto">
            ...
            {equipment.map(eq => {
              ...
              return (
                <div key={eq.id} className="bg-white rounded-lg shadow-md p-6">
                  <h2 className="text-xl font-bold text-gray-900 mb-4">{eq.name} ({eq.id})</h2>
        ...
        {activeTab === 'predictions' && (
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="space-y-4">
              {predictions.sort((a, b) => a.daysToFailure - b.daysToFailure).map((pred, idx) => (
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex gap-4">
                      <span className="text-sm">
                        Temp: <span className={pred.trends.temperature === 'increasing' ? 'text-red-600' : 'text-green-600'}>
                          {pred.trends.temperature}
                        </span>
                      </span>
                      <span className="text-sm">
                        Vibration: <span className={pred.trends.vibration === 'increasing' ? 'text-red-600' : 'text-green-600'}>
                          {pred.trends.vibration}
                        </span>
                      </span>
                      <span className="text-sm">
                        Pressure: <span className={pred.trends.pressure === 'increasing' ? 'text-red-600' : 'text-green-600'}>
